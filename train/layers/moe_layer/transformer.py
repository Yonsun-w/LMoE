...
class Moe_Atten(nn.Module):
    ...
    def forward(self, obs, wrf):
        # 先将输入传递给patchembed [b,3,159,159]==>[b,81,768]
        ...
        for i in range(true_time):
            ...
            list1 = [obs_x, wrf_x]
            list2 = [wrf_x, obs_x]
            o_x = self.blocks_o(list1)[0]
            w_x = self.blocks_w(list2)[0]
            o_x = self.norm(o_x)
            w_x = self.norm(w_x)
        ...
        wrf = torch.cat(output_w,dim=0)
        obs = torch.cat(output_o,dim=0)
        return obs,wrf
...
if __name__ == '__main__':
    ...
    a = [x,x]
    x,y = a[0],a[1]
    # my_block = nn.Sequential(*[moe_encoder_block(dim=768) for _ in range(12)])
    # test = moe_encoder_block(dim=768)
    # print(my_block(my_block(a))[0].shape)
    # print(a.shape)
